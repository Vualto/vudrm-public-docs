...
| `geo_whitelist`     | string array | Array of 3 letter country code (ISO format) | Fairplay, Widevine | A list of 3 letter country codes, in ISO format, for all countries that are allowed access. 
...
### Geo whitelisting
If `geo_whitelist` has been added to the VUDRM token's policy, when a license is requested the geo location of the clients ip will be checked. If the country is in the whitelist the license request will be successful. If the country is not in the whitelist the request will be denied. E.g. if the VUDRM tokens policy is `{ "geo_whitelist":[ "gbr", "deu" ] }` only users from the UK and Germany will be able to make successful license request.
