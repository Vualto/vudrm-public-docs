...
| `polbegin`         | datetime | DD-MM-YYYY HH:mm:ss       | ALL            | Policy Begin. The start date for the license. Default value depends on the DRM provider. The default timezone for this value is Europe/London. This value comes from [this database](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) and if desired we can update your timezone to be one of the majority of these timezones.                                                    |
