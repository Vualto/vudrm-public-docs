...
| `firstplayback`    | integer  | Seconds                   | PlayReady      | Once playback initiated the user will have this window to complete playback. Once the window completes the license will expire.  **If you wish to use `firstplayback` and `liccache` in the same policy, `liccache` must be set to `yes`.** | 
